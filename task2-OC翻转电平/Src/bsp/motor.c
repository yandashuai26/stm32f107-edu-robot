...
void motor_init()
{
	...
	// 固定占空比且频率可调 , 步进电机驱动芯片只需要占空比保持 50%
...
// 改变速度全部在 TIM5_IRQHandler 中 , 其他地方只改变标志位
        ...
        if (motor_running)
        {
            ...
            if (speed_changing)
            {
                ...
                if (current_hz == target_hz)
                {
					...
					// 如果是停止命令（目标频率0，减速完成后停止电机）
                    if (target_hz == 0 && current_hz == 0)
                    {
                        motor_running = 0;
