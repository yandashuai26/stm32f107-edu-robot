...
static volatile uint16_t USART1_RxIndex = 0;    // 中断函数中使用 ,记录接收到的字符数量
static volatile uint8_t  USART1_RxFinished = 0; // 1 表示接收到一个完整数据包
static volatile uint16_t USART1_RxCount = 0;  // 接收到的字符数量
...
// ///把PA9引脚初始化为复用推挽输出 把PA10引脚初始化为浮空输入
// // 把PA12引脚初始化为推挽输出  默认接收模式
...
// * 形    参: _pBuf : 参与校验的数据
// *           _usLen : 数据长度
// * 返 回 值: 16位校验值， 遵循Modbus 先发送低字节，再发送高字节。
*
// *   所有可能的CRC值都被预装在两个数组当中，当计算报文时可以简单的索引即可；
// *   一个数组包含16位CRC的高位字节，另一个数组含有低位字节的值。
// *   按照索引访问数组的方式提供了比对每一个报文重新计算新的CRC的方法。
*********************************************************************************************************
*/
...
// // 使用485发送指令
// // uAddr 设备地址  opr 功能码 03-->读  06-->写 uReg 寄存器地址
...
// // crc校验
    ...
    tx_date[6] = crc >> 8;  // crc占16位,取高半位
    tx_date[7] = crc & 0xFF;  // 取低半位
...
void USART1_IRQHandler(void)
{
    ...
    if (USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)
    {
        ...
        temp = USART1->DR;
        (void)temp;//temp没用到,但是写它是为了清除硬件标志位

        USART1_RxCount = USART1_RxIndex;//字符数量

        USART1_RxFinished = 1;//标志位置1 , 接收到一帧数据
        USART1_RxIndex = 0;//缓冲区归零
        // 清除中断标志位
